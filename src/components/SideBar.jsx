    ...
    <div
      className={`${
        showSideBar
          ...
          : "w-0 -translate-x-64 min h-[100vh] overflow-hidden hover:overflow-y-auto shadow-xl sticky top-0 transition-all"
      }`}
    >
      ...
      <div className="p-8 flex flex-col gap-3">
        {sidebarData.map((items, index) => (
          <div key={index} className="cursor-pointer">
            <div className="flex justify-between items-center font-bold text-xl text-gray-500">
              <h1>{items.header}</h1>
            ...
            {items.pages.map((item, index) => (
              <div
                ...
                className={`transition-all duration-300 ${
                  items.header === "Applications" && !showApp
                    ? "hidden opacity-0 -translate-y-10 pointer-events-none absolute"
                    : "opacity-100 pointer-events-auto"
                }`}
              >
                <Link href={item.link} onClick={() => setHeader(item.name)}>
                  <div
                    className={`${
                      pathname === item.link
        ...
        {sidebarAdmin.map((items, index) => (
          <div key={index} className="cursor-pointer">
            <h1 className="font-bold text-xl text-gray-500">{items.header}</h1>
            {items.pages.map((item, index) => (
              <Link
                key={index}
                href={item.link}
                ...
                <div
                  className={`${
                    pathname == item.link
                      ? "bg-blue-500 text-white font-bold"
                      : ""
                  } flex items-center gap-2 mt-2 ml-3 hover:bg-blue-500 hover:text-white 
              px-3 py-2 rounded-xl hover:font-bold`}
                >
                  {item.icon}
                  <h3>{item.name}</h3>
                </div>
        ...
        <Link href={"/login"}>
          <div
            className=" font-semibold flex items-center gap-2 mt-2 hover:bg-blue-500 hover:rounded-xl hover:text-white 
              px-3 py-2 hover:font-bold border-t-2 border-gray-400"
          >
            <FiLogOut />
            <h3>Logout</h3>
          </div>
